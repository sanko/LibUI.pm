...
The callback is not triggered when calling C<uiMultilineEntrySetText( ... )> or
C<uiMultilineEntryAppend( ... )>.
...
A C<LibUI::DrawMatrix> transform applied to the brush, or C<undef> for no
transform.
...
Creates a new table with the given parameters. The C<Model> field is a L<uiTableModel>,
and C<RowBackgroundColorModelColumn> specifies a column index whose color value will be
used as the row background color (use C<-1> for none).
...
C<:constants> tag or all at once with C<:all>.
...
Convenience functions that reduce boilerplate. Import with the C<:helpers> tag
or C<:all>.
...
Returns a C<DrawBrush> hashref configured as a solid-color brush. C<$a> (alpha)
defaults to C<1.0>.
...
Formats a C<struct tm> hashref (as returned by C<uiDateTimePickerTime>) into a
human-readable string like C<"Sun Jul  6 14:30:05 2026">.

=head1 LibUI::Matrix

Object-oriented wrapper around C<uiDrawMatrix>. Encapsulates the 6-field
...
Applies a rotation (in radians) around point C<($x, $y)>. Returns C<$self> for
chaining.
...
Applies the matrix to a draw context via C<uiDrawTransform>. Returns C<$self>
for chaining.

=head1 DATETIME PICKER WRAPPERS

The C<uiDateTimePickerTime> and C<uiDateTimePickerSetTime> functions are
wrapped to work with Perl-friendly data structures instead of raw C<struct tm>
buffers.
...
When using the epoch form, C<localtime()> is used internally and C<tm_isdst>
is set to C<-1> (let the system decide).
...
allocation and cleanup (no need for C<calloc>, C<sizeof>, C<cast>, or
C<uiFreeFontButtonFont>).
...
Same structure as C<uiFontButtonFont>.
...
When passed a model pointer directly, C<RowBackgroundColorModelColumn> defaults
to C<-1> (disabled).

=head1 AREA HANDLER ALIASES

C<uiNewArea> and C<uiNewScrollingArea> accept callback aliases with C<on_>
...
=item C<on_draw> or C<Draw>

=item C<on_mouse> or C<on_mouse_move> or C<MouseEvent>

=item C<on_crossed> or C<on_mouse_crossed> or C<MouseCrossed>

=item C<on_drag> or C<on_drag_broken> or C<DragBroken>

=item C<on_key> or C<on_key_event> or C<KeyEvent>

=back

The C<Draw>, C<MouseEvent>, and C<KeyEvent> callbacks receive typed hashrefs
instead of raw pointers. See L</"AREA CALLBACK STRUCTS">.
